      ...
      *                                                                 00180000
       77 WS-BEGIN              PIC X(17) VALUE 'CIOCCA01 WS BEGIN'.    00190001
       01 WS-VAR.                                                       00200000
      ...
      *                                                                 00460000
       01 WS-COMMAREA.                                                  00470000
          ...
          05 WS-CUSTOMER-NAME   PIC X(30).                              00494010
       01 WS-COMMAREA-SELF.                                             00495019
          05 WS-FIRST-SEND      PIC X(1).                               00496015
          05 WS-OPTION          PIC 9(3).                               00497015
       77 WS-END                PIC X(15) VALUE 'CIOCCA01 WS END'.      00500001
      *                                                                 00510000
      ...
      *                                                                 00780000
       1000-INIT.                                                       00790000
            IF LK-FIRST-SEND = 'Y'                                      00800006
               MOVE LOW-VALUES TO CICA01O                               00810003
               SET WS-MAP-ERASE TO TRUE                                 00820000
               PERFORM 3030-SEND-MAP                                    00830000
                  THRU 3030-SEND-MAP-EXIT                               00840000
      * NOT FIRST SHOW                                                  00850000
            ELSE                                                        00860000
                  MOVE LOW-VALUES TO CICA01I                            00880003
                  EXEC CICS RECEIVE MAP('CICA01')                       00890003
                                   MAPSET('CICA01')                     00900003
                                   INTO(CICA01I)                        00910003
       ...
       3000-MAIN-PROCESS.*>IMPORTANT                                            
            EVALUATE EIBAID                                             01310000
                WHEN DFHPF1                                             01320026
                     EXEC CICS                                          01330026
                          XCTL PROGRAM('CIOCCA00')                      01340027
                               RESP(WS-RESP-CODE)                       01350026
                     END-EXEC                                           01360026
                     IF WS-RESP-CODE NOT = DFHRESP(NORMAL)              01370026
                        MOVE 'PROGRAM CIOCCI00 IS NOT AVAILABLE'        01380026
                                TO MSGO                                 01390026
                ...
                WHEN DFHPF9                                             01650000
                     MOVE LOW-VALUES TO CICA01O                         01660003
      ...
      *                                                                 01920000
       3010-CHECK-INPUT.                                                01930000
            IF COMMUL NOT = 0                                           01931012
               INITIALIZE CIMENU-REC                                    01940012
               MOVE COMMUI TO CIMENU-TRANSID                            01940112
               ...
               EVALUATE WS-RESP-CODE                                    01940812
                   WHEN DFHRESP(NORMAL)                                 01940912
                        EXEC CICS                                       01941012
                             ...
                             COMMAREA(CIMENU-TRANSID)                   01941228
            ...
            END-IF                                                      01944012
            IF (CNAMEL = 0 OR IDTYPEL = 0 OR IDNUML = 0)                01945016
            ...
            EVALUATE TRUE                                               02730000
                WHEN WS-MAP-ERASE                                       02740000
                     EXEC CICS SEND                                     02750000
                          MAP('CICA01')                                 02760003
                          MAPSET('CICA01')                              02770003
                          FROM(CICA01O)                                 02780003
                          ERASE                                         02790000
                     END-EXEC                                           02800000
                WHEN WS-MAP-DATAONLY                                    02810000
                     EXEC CICS SEND                                     02820000
                          MAP('CICA01')                                 02830003
                          MAPSET('CICA01')                              02840003
                          FROM(CICA01O)                                 02850003
                          DATAONLY                                      02860000
                     END-EXEC                                           02870000
      ...
      *                                                                 03140000
       5020-RETURN-TRANS.                                               03150000
            INITIALIZE WS-COMMAREA-SELF                                 03151017
            MOVE 'N' TO WS-FIRST-SEND OF WS-COMMAREA-SELF               03152017
            MOVE LK-OPTION TO WS-OPTION OF WS-COMMAREA-SELF             03153018
            EXEC CICS RETURN TRANSID('CIA1')                            03160020
                      COMMAREA(WS-COMMAREA-SELF)                        03170018
            END-EXEC                                                    03180000
            .                                                           03190000
