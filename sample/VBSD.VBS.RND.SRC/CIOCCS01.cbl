      ...
      *                                                                 00180000
       77 WS-BEGIN              PIC X(17) VALUE 'CIOCCS01 WS BEGIN'.    00190030
       01 WS-VAR.                                                       00200000
      ...
      *                                                                 00460000
       01 WS-COMMAREA.                                                  00470000
          ...
          05 WS-ID-NUMBER       PIC 9(18).                              00492007
          05 WS-ID-TYPE         PIC 9(3).                               00493007
       01 WS-COMMAREA-SELF.                                             00495038
          05 WS-FIRST-SEND      PIC X(1).                               00496038
          05 WS-OPTION          PIC 9(3).                               00497038
       77 WS-END                PIC X(15) VALUE 'CIOCCS01 WS END'.      00500030
      *                                                                 00510000
       LINKAGE SECTION.                                                 00520000
       01 DFHCOMMAREA.                                                  00530034
      *   05 LK-FIRST-SEND      PIC X(1).                               00550043
      *   05 LK-OPTION          PIC 9(3).                               00551043
      ...
      *                                                                 00780000
       1000-INIT.                                                       00790000
      *     IF LK-FIRST-SEND = 'Y'                                      00800043
      *        MOVE LOW-VALUES TO CICS01O                               00810043
      *        SET WS-MAP-ERASE TO TRUE                                 00820043
      *        PERFORM 3030-SEND-MAP                                    00830043
      *           THRU 3030-SEND-MAP-EXIT                               00840043
      * NOT FIRST SHOW                                                  00850041
      *     ELSE                                                        00860043
      *           MOVE LOW-VALUES TO CICS01I                            00880043
      *           EXEC CICS RECEIVE MAP('CICS01')                       00890043
      *                            MAPSET('CICS01')                     00900043
      *                            INTO(CICS01I)                        00910043
      *                            RESP(WS-RESP-CODE)                   00920043
      *           END-EXEC                                              00930043
      *     END-IF                                                      00950043
      *     .                                                           00960043
            IF EIBCALEN = 0                                             00961043
               MOVE LOW-VALUES TO CICS01O                               00962043
               SET WS-MAP-ERASE TO TRUE                                 00963043
               PERFORM 3030-SEND-MAP                                    00964043
                  THRU 3030-SEND-MAP-EXIT                               00965043
      * NOT FIRST SHOW                                                  00966034
            ELSE                                                        00967043
               IF SDCA-CICS-SECONDENTER                                 00968043
                  MOVE LOW-VALUES TO CICS01I                            00969043
                  EXEC CICS RECEIVE MAP('CICS01')                       00969143
                                   MAPSET('CICS01')                     00969243
                                   INTO(CICS01I)                        00969343
                                   RESP(WS-RESP-CODE)                   00969443
                  END-EXEC                                              00969543
               END-IF                                                   00969643
            END-IF                                                      00969743
            .                                                           00969843
      ...
      *                                                                 01260000
       2000-PRE-PROCESSING-EXIT.                                        01270000
            EXIT.                                                       01280000
      *                                                                 01290000
       3000-MAIN-PROCESS.*>IMPORTANT                                                 01300000
            EVALUATE EIBAID                                             01310000
                WHEN DFHPF1                                             01320026
                     EXEC CICS                                          01330026
                          XCTL PROGRAM('CIOCCS00')                      01340028
                               RESP(WS-RESP-CODE)                       01350026
                     END-EXEC                                           01360026
                     IF WS-RESP-CODE NOT = DFHRESP(NORMAL)              01370026
                        MOVE 'PROGRAM CIOCCS00 IS NOT AVAILABLE'        01380028
                                TO MSGO                                 01390026
      ...
      *                                                                 01920000
       3010-CHECK-INPUT.*>IMPORTANT                                             
            ...
            .                                                           02150000
      *                                                                 02160000
      * IDNUM MAY NOT EXIST, NEED TO VBS.CI.APPLICAN.IN1 ?????          02161036
      ...
      *                                                                 02190000
       3020-XCTL.                                                       02200000
            ...
            MOVE IDTYPEI TO WS-ID-TYPE                                  02250016
            MOVE IDNUMI TO WS-ID-NUMBER                                 02260016
            .                                                           02650000
            EXEC CICS                                                   02651016
                 XCTL PROGRAM('CIOCCS02')                               02652028
      ...
      *     MOVE 'N' TO LK-FIRST-SEND                                   02890018
            MOVE '1' TO WS-ENTER-FLAG                                   02891034
      ...
      *     INITIALIZE WS-COMMAREA-SELF                                 03151048
      *     MOVE 'N' TO WS-FIRST-SEND OF WS-COMMAREA-SELF               03152048
      *     MOVE '1'    TO WS-OPTION OF WS-COMMAREA-SELF                03153048
            EXEC CICS RETURN TRANSID('CIB1')                            03160042
                      COMMAREA(WS-ENTER-FLAG)                           03171048
            END-EXEC                                                    03180000
            .                                                           03190000
